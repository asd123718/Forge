...
export enum ChatConfiguration {
	...
	DefaultToCodexHarness = 'chat.defaultToCodexHarness',
 ...
 * when the agent host is enabled, Forge can prefer Codex or the Copilot
 * harness through its default-harness settings. It falls back to the local
 * harness when enabled, or to the first visible non-local provider.
 */
export function getComputedDefaultSessionType(
	...
	if (agentHostEnabled && configurationService.getValue<boolean>(ChatConfiguration.DefaultToCodexHarness)) {
