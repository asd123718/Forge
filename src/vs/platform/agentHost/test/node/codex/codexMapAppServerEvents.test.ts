...
suite('codexMapAppServerEvents', () => {
	...
	test('shell-written file edits remain attached to command completion', () => {
		...
		mapItemStarted(state, {
			item: {
				type: 'commandExecution', id: 'cmd_write', command: 'Set-Content app.ts value', cwd: '/tmp', processId: null,
				source: 'agent' as never, status: 'inProgress' as never, commandActions: [], aggregatedOutput: null, exitCode: null, durationMs: null,
			} as never,
			threadId: 'thr_1', turnId: 'turn_a', startedAtMs: 0,
		});
		const fileEdit = {
		...
		const actions = mapItemCompleted(state, {
			item: {
				type: 'commandExecution', id: 'cmd_write', command: 'Set-Content app.ts value', cwd: '/tmp', processId: null,
				source: 'agent' as never, status: 'completed' as never, commandActions: [], aggregatedOutput: '', exitCode: 0, durationMs: 1,
			} as never,
			threadId: 'thr_1', turnId: 'turn_a', completedAtMs: 0,
		}, [fileEdit]);
		assert.strictEqual(actions[0].type, ActionType.ChatToolCallComplete);
		assert.deepStrictEqual((actions[0] as { result: { content: unknown[] } }).result.content, [fileEdit]);
	});
	...
	test('streaming patch starts the file tool call before item/started', () => {
		...
		const patchActions = mapFileChangePatchUpdated(state, {
			threadId: 'thr_1', turnId: 'turn_a', itemId: 'file_live', changes: [...changes],
		});
		const duplicateStart = mapItemStarted(state, {
			item: { type: 'fileChange', id: 'file_live', changes, status: 'inProgress' } as never,
			threadId: 'thr_1', turnId: 'turn_a', startedAtMs: 0,
		});
	...
	test('dynamicToolCall item omits the Client contributor for write_file', () => {
		const toolSet = new ActiveClientToolSet();
		toolSet.set('win-7', [{ name: 'get_magic_word' }]);
		const state = createCodexSessionMapState(new Set(['write_file']), toolSet);
		const startActions = mapItemStarted(state, {
			item: { type: 'dynamicToolCall', id: 'dyn_write', namespace: null, tool: 'write_file', arguments: { path: 'game.js', contents: 'full' }, status: 'inProgress', contentItems: null, success: null, durationMs: null } as never,
			threadId: 'thr_1', turnId: 'turn_a', startedAtMs: 0,
		});
		const start = startActions[0] as { type: ActionType; toolName: string; displayName?: string; contributor?: { kind: ToolCallContributorKind; clientId: string } };
		assert.deepStrictEqual({
			type: start.type,
			toolName: start.toolName,
	...
	test('write_file dynamicToolCall complete includes FileEdit snapshots for live preview', () => {
		const state = createCodexSessionMapState(new Set(['write_file']));
		mapItemStarted(state, {
			item: { type: 'dynamicToolCall', id: 'dyn_write', namespace: null, tool: 'write_file', arguments: { path: 'game.js', contents: 'full' }, status: 'inProgress', contentItems: null, success: null, durationMs: null } as never,
			threadId: 'thr_1', turnId: 'turn_a', startedAtMs: 0,
		});
		...
		const completeActions = mapItemCompleted(state, {
			item: { type: 'dynamicToolCall', id: 'dyn_write', namespace: null, tool: 'write_file', arguments: { path: 'game.js', contents: 'full' }, status: 'completed', contentItems: [{ type: 'inputText', text: 'Wrote game.js (4 characters).' }], success: true, durationMs: 5 } as never,
			threadId: 'thr_1', turnId: 'turn_a', completedAtMs: 0,
		}, [fileEdit]);
