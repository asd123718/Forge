...
async function main(): Promise<void> {
	...
	if (!options.quiet) {
		...
		// so leftover Copilot DI seams still resolve.
		...
		// Forge exposes one agent provider: Codex. Built installers bundle the
		// Codex npm package directly, so treat that package as an available SDK
		// in addition to the product-download and explicit override paths.
		const codexSdkAvailable = !environmentService.isBuilt
			|| agentSdkDownloader.isAvailable(CodexSdkPackage)
			|| (await resolveCodexDevSdkRoot()) !== undefined;
