...
export class CodexFileEditObserver extends Disposable {
	...
	begin(itemId: string, workingDirectory: URI | undefined, changes: readonly FileUpdateChange[]): void {
		for (const change of changes) {
			this._ensureFile(itemId, workingDirectory, change.path);
		}
	}

	async snapshot(turnId: string, toolCallId: string, itemId: string, workingDirectory: URI | undefined, changes: readonly FileUpdateChange[]): Promise<readonly ToolResultFileEditContent[]> {
		const edits: ToolResultFileEditContent[] = [];
		for (const change of changes) {
			const observed = this._ensureFile(itemId, workingDirectory, change.path);
			try {
				await observed.start;
				const beforeText = await observed.beforeText;
				const afterText = previewFileChange(beforeText, change);
				const edit = await this._tracker.snapshotEditContent(turnId, toolCallId, observed.path, afterText);
				if (edit) {
					edits.push(edit);
				}
			} catch (error) {
	...
	async completeShell(turnId: string, toolCallId: string, itemId: string): Promise<readonly ToolResultFileEditContent[]> {
		...
		for (const path of paths) {
			const before = observed.before.get(path) ?? { existed: false, content: '' };
			const current = after.get(path) ?? { existed: false, content: '' };
	...
	async snapshotTurnDiff(turnId: string, toolCallId: string, workingDirectories: readonly URI[], diff: string): Promise<readonly ToolResultFileEditContent[]> {
		...
		for (const file of parseGitTurnDiff(diff)) {
			try {
				const path = await this._resolveTurnDiffPath(file.path, file.afterExists, workingDirectories);
				const afterText = file.afterExists ? await this._readFile(path) : '';
				const beforeText = file.beforeExisted ? applyUnifiedDiff(afterText, invertUnifiedDiff(file.patch)) : '';
	...
	private async _resolveTurnDiffPath(relativePath: string, afterExists: boolean, workingDirectories: readonly URI[]): Promise<string> {
		...
		for (const root of roots) {
			...
			if (!afterExists || await this._fileService.exists(URI.file(candidate))) {
				return candidate;
			}
		}
	...
	private _ensureFile(itemId: string, workingDirectory: URI | undefined, filePath: string): IObservedFileEdit {
		...
		if (!observed) {
			const beforeText = this._readFile(path);
			observed = { path, beforeText, start: this._tracker.trackEditStart(path) };
...
async function readShellSnapshot(path: string): Promise<IShellFileSnapshot> {
	try {
		const stat = await fs.stat(path);
		if (!stat.isFile() || stat.size > shellSnapshotMaxFileBytes) {
			return { existed: stat.isFile(), content: '' };
		}
...
async function snapshotDirectory(root: string, snapshots: Map<string, IShellFileSnapshot>, maxFiles: number, maxBytes: number): Promise<{ files: number; bytes: number }> {
	let files = 0;
	let bytes = 0;
	const pending = [root];
	while (pending.length > 0 && files < maxFiles && bytes < maxBytes) {
		...
		for (const entry of entries) {
			...
			if (!snapshot.existed || snapshot.content.length === 0) {
				continue;
			}
...
/** Build the right-hand diff content without writing to the workspace. */
export function previewFileChange(beforeText: string, change: FileUpdateChange): string {
	switch (change.kind.type) {
		case 'add': return change.diff;
		case 'delete': return '';
		case 'update': return applyUnifiedDiff(beforeText, stripMoveTrailer(change.diff, change.kind.move_path));
	}
}
...
export function applyUnifiedDiff(original: string, diff: string): string {
	...
	for (let index = 0; index < diffLines.length; index++) {
		...
		sawHunk = true;
		const hunkStart = Math.max(0, Number(header[1]) - 1);
		...
		for (index++; index < diffLines.length && !diffLines[index].startsWith('@@ '); index++) {
			...
			if (line.startsWith(' ')) {
				result.push(originalLines[originalIndex] ?? line.slice(1));
				originalIndex++;
				...
				// Metadata for the preceding line; no content of its own.
			}
		}
		index--;
	}
	...
	return result.join('\n') + (originalEndsWithNewline || diff.endsWith('\n') ? '\n' : '');
}
