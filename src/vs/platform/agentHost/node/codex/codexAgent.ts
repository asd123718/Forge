...
import { discoverCodexLocalModels, type ICodexDiscoveredLocalModel } from './codexLocalModelDiscovery.js';
import { CodexSessionConfigKey, CODEX_DEFAULT_PERMISSIONS_PRESET, CODEX_PERMISSIONS_PRESETS, collaborationModeKind, inferCodexPermissionsPreset, migrateCodexPermissionValues, narrowAdditionalDirectories, narrowBoolean, narrowCodexPermissionsPreset, narrowPersonality, narrowReasoningEffort, narrowReasoningSummary, narrowWebSearchMode, resolveCodexPermissions, resolveCodexPermissionsPreset, type CodexApprovalPolicy, type CodexPermissionsPreset, type ICodexResolvedPermissions } from './codexSessionConfigKeys.js';
...
import type { TurnDiffUpdatedNotification } from './protocol/generated/v2/TurnDiffUpdatedNotification.js';
...
import type { ErrorNotification } from './protocol/generated/v2/ErrorNotification.js';
...
import type { ConfigEdit } from './protocol/generated/v2/ConfigEdit.js';
...
interface ICodexSession {
	...
	/** Approval/sandbox/reviewer axes the current thread was started with. */
	materializedPermissionsSig: string | undefined;
...
export class CodexAgent extends Disposable implements IAgent {
	...
	private readonly _fileEditObservers = this._register(new DisposableMap<string, CodexFileEditObserver>());
	...
	private readonly _fileEventDispatches = new Map<string, Promise<void>>();
	/**
	...
	private readonly _modelProviderApiKeys = new Map<string, string>();
	...
	private readonly _localModelDiscoveryCache = new Map<string, { readonly expiresAt: number; readonly promise: Promise<readonly ICodexDiscoveredLocalModel[]> }>();
	...
	private readonly _configuredCodexHome: string | undefined;
	...
	private _resetSessionForModelProviderChange(session: ICodexSession, modelProvider: string): void {
		...
		session.materializedPermissionsSig = undefined;
	...
	async authenticate(resource: string, token: string): Promise<boolean> {
		...
		if (configuredProvider) {
			...
			if (previous !== (token || undefined)) {
				// Root config and credential updates travel as adjacent protocol
				// messages. Let any config.toml write finish before replacing the
				// app-server so the write cannot be aborted midway.
				...
				void pendingConfigurationWrite.finally(() => {
					...
					if (current === (token || undefined) && connectionGeneration === this._connectionGeneration) {
	 ...
	 */
	private _permissionAxisDefaults(): { approvalPolicy: CodexApprovalPolicy; sandboxMode: SandboxMode } {
	...
	private _permissionsSignature(configResource: URI): string {
	...
	private _hasExistingChatGPTSetup(): boolean {
		...
		return detectExistingCodexChatGPTSetup(
			...
			process.env,
			this._codexHome,
		) || (this._configuredCodexHome === undefined && detectExistingCodexChatGPTSetup(
			this._environmentService.userHome.fsPath,
			process.env,
	...
	private async _refreshCodexModels(): Promise<void> {
		try {
			...
			if (localKind) {
				...
				this._codexModels = [...modelIds].map(([model, name]): IAgentModelInfo => ({
	...
	private _discoverLocalModels(kind: 'ollama' | 'lmstudio', baseUrl: string): Promise<readonly ICodexDiscoveredLocalModel[]> {
		...
		this._localModelDiscoveryCache.set(key, { expiresAt: Date.now() + 5 * 60_000, promise });
		return promise;
	}
	...
	private async _startConnection(): Promise<IConnectionReady> {
		...
		this._register(client.onNotification('turn/completed', params => this._queueFileEvent(params.threadId, () => { this._dispatchTurnCompleted(params); })));
	...
	private _hostServerToolNames(): Set<string> {
		return new Set([...(this._serverToolHost?.toolNames ?? []), CODEX_WRITE_FILE_TOOL_NAME]);
	}

	private _workspaceRoots(session: ICodexSession): URI[] {
	...
	private async _handleDynamicToolCallRpc(params: DynamicToolCallParams): Promise<ServerRequestHandlerResult<DynamicToolCallResponse>> {
		...
		const session = (sessionId ? this._sessions.get(sessionId) : undefined)
			?? this._subagentsByThreadId.get(params.threadId)?.session;
		if (!session) {
	...
	private async _handleWriteFileTool(session: ICodexSession, params: DynamicToolCallParams): Promise<ServerRequestHandlerResult<DynamicToolCallResponse>> {
		const observer = this._fileEditObserver(session);
		try {
			// item/started is queued on the file-event chain; wait so the host
			// toolCallId exists before we emit a live FileEdit snapshot.
	...
	private _emitForThread(threadId: string, session: ICodexSession, action: SessionAction | ChatAction): void {
	...
	private _handleErrorNotification(session: ICodexSession, params: ErrorNotification): (SessionAction | ChatAction)[] {
	...
	private _handleTurnCompletedNotification(session: ICodexSession, params: TurnCompletedNotification): (SessionAction | ChatAction)[] {
		...
		return [{ type: ActionType.ChatActivityChanged, activity: undefined }, ...out];
	}
	...
	private _fileEditObserver(session: ICodexSession): CodexFileEditObserver | undefined {
		if (!session.threadId) {
			return undefined;
		}
	...
	private _forgeModelsFilePath(): string {
	...
	private _readModelsConfiguration(config: Record<string, unknown>): ICodexModelsConfig {
		...
		const providers: ICodexModelProviderEntry[] = [];
		if (rawProviders && typeof rawProviders === 'object' && !Array.isArray(rawProviders)) {
			for (const [id, raw] of Object.entries(rawProviders as Record<string, unknown>)) {
	...
	private async _writeProviderConfiguration(key: string, value: unknown): Promise<void> {
		...
		if (key === 'codex.permissionsPreset') {
			...
			if (preset === 'full-access') {
				// Codex Desktop migrates `[windows] sandbox = "unelevated"`, which keeps
				// RestrictedToken wrapping even after the UI selects Full Access.
			...
			edits = [{ keyPath: key === 'codex.personality' ? 'personality' : 'auto_review.policy', value: value as string, mergeStrategy: 'replace' }];
		}
		...
		if (key === 'codex.permissionsPreset') {
			...
			for (const session of this._sessions.values()) {
				...
				if (session.threadId !== undefined && !session.firstTurnSent && !session.disposed) {
	...
	private async _writeModelsConfiguration(value: unknown): Promise<void> {
		...
		// model/list owns a process-level model manager, so a config reload is
		// insufficient when the provider changes.
		this._disposeConnection();
		void this._queueModelRefresh();
	}
	 ...
	 */
	private _queueFileEvent(threadId: string, task: () => void | Promise<void>): void {
	...
	private async _dispatchTurnDiffUpdated(params: TurnDiffUpdatedNotification): Promise<void> {
		...
		const turnId = this._hostTurnId(session, params.turnId);
		const toolCallId = session.mapState.turnDiffToolCall?.turnId === turnId
			? session.mapState.turnDiffToolCall.toolCallId
			: generateUuid();
	...
	private async _dispatchItemStarted(params: ItemStartedNotification): Promise<void> {
		const subagent = this._subagentsByThreadId.get(params.threadId);
		const sessionId = this._sessionIdByThreadId.get(params.threadId);
		const session = subagent?.session ?? (sessionId ? this._sessions.get(sessionId) : undefined);
		if (!session) {
	...
	private async _dispatchItemCompleted(params: ItemCompletedNotification): Promise<void> {
		const subagent = this._subagentsByThreadId.get(params.threadId);
		if (subagent) {
			const entry = subagent.session.mapState.itemToToolCall.get(params.item.id);
			const fileEdits = await this._fileEditsForCompletedItem(subagent.session, params, entry);
			const actions = mapItemCompleted(subagent.session.mapState, this._withHostTurnId(subagent.session, params), fileEdits);
			for (const action of actions) {
				this._fireSubagent(subagent, action);
			}
			return;
	...
	private async _fileEditsForCompletedItem(
		session: ICodexSession,
		params: ItemCompletedNotification,
		entry: { readonly turnId: string; readonly toolCallId: string } | undefined,
	): Promise<readonly ToolResultFileEditContent[]> {
		if (!entry) {
			return [];
		}
		const observer = this._fileEditObserver(session);
	...
	private _createSubagentSession(parent: ICodexSession, childThreadId: string): ICodexSession {
		...
		return {
			...
			materializedPermissionsSig: undefined,
	 ...
	 */
	private async _refreshMcpInventory(client: ICodexAppServerClient, preserveMissingReadyServer?: string): Promise<void> {
	...
	private _handleMcpStartupStatus(client: ICodexAppServerClient, name: string, status: McpServerStartupState, error: string | null): void {
		...
		if (status === 'ready') {
			// The ready notification is authoritative. Publish it immediately and
			// preserve it if the concurrently refreshed list has not caught up yet.
	...
	getInheritedChatConfig(config: Readonly<Record<string, unknown>>): Record<string, unknown> | undefined {
		const inherited: Record<string, unknown> = migrateCodexPermissionValues(config, this._permissionAxisDefaults());
...
/**
 * Resolve the on-disk directory that holds `@openai/codex-<target>` for a
 * given SDK root. Packaged desktop builds unpack native Codex binaries beside
 * the ASAR archive under `node_modules.asar.unpacked`; dev checkouts and the
 * on-demand SDK cache use a plain `node_modules` tree.
 */
export function resolveCodexNodeModulesDirName(sdkRoot: string, codexTarget: string, triple: string, binaryName: string): 'node_modules' | 'node_modules.asar.unpacked' {
...
export function resolveCodexBinaryPath(sdkRoot: string, codexTarget: string, triple: string, binaryName: string): string {
 ...
 * onto), or undefined when the package can't be resolved. `@openai/codex`
 * is a production dependency, so this also works in built products that
 * ship the package under `resources/app/node_modules`. `@openai/codex`
 * declares no `exports` map, so its `package.json` is resolvable.
 *
