...
import { MessageKind, ResponsePartKind, ToolCallConfirmationReason, ToolCallContributorKind, ToolResultContentType, TurnState, type ErrorInfo, type ToolResultFileEditContent } from '../../common/state/sessionState.js';
...
export interface ICodexSessionMapState {
	...
	/** Synthetic live file-edit call fed by Codex's cumulative turn diff. */
	turnDiffToolCall: { readonly turnId: string; readonly toolCallId: string; content: readonly ToolResultFileEditContent[] } | undefined;
	/**
...
export function resetCodexTurnMapState(state: ICodexSessionMapState): void {
	...
	state.turnDiffToolCall = undefined;
...
function completeTurnDiffToolCall(state: ICodexSessionMapState, success: boolean, message: string): (SessionAction | ChatAction)[] {
...
/**
 * Starts the file-edit tool call on the first structured patch snapshot. Codex
 * emits `fileChange/patchUpdated` while tool arguments are still streaming,
 * before the normal `item/started`; the later start notification therefore
 * becomes an idempotent no-op.
 */
export function mapFileChangeStarted(
	state: ICodexSessionMapState,
	turnId: string,
	itemId: string,
...
/** Maps cumulative turn diffs, including shell-written files, into live file edits. */
export function mapTurnDiffUpdated(
	state: ICodexSessionMapState,
	turnId: string,
	toolCallId: string,
	fileEdits: readonly ToolResultFileEditContent[],
...
export function mapItemCompleted(
		...
		const completion: (SessionAction | ChatAction)[] = [
			{
				...
				result: {
					...
					content: output || fileEdits.length > 0
						? [
							...(output ? [{ type: ToolResultContentType.Text as const, text: output }] : []),
							...fileEdits,
						]
						: undefined,
	...
	if (params.item.type === 'dynamicToolCall') {
		...
		const serverDisplay = success ? (getServerToolDisplay(entry.toolName, params.item.arguments, { text: output, success }) ?? getWriteFileToolDisplay(entry.toolName, params.item.arguments, { text: output, success })) : undefined;
		return [{
			type: ActionType.ChatToolCallComplete,
			turnId: entry.turnId,
			toolCallId: entry.toolCallId,
...
/** Maps a live app-server error notification into visible retry or failure UI. */
export function mapErrorNotification(params: ErrorNotification, turnId: string, duration: number): ChatAction[] {
	if (params.willRetry) {
		return [{
