...
export class FileEditTracker {
	 ...
	 */
	abandonEdit(filePath: string): void {
	 ...
	 */
	async snapshotEdit(turnId: string, toolCallId: string, filePath: string): Promise<ToolResultFileEditContent | undefined> {
		const pending = this._pendingEdits.get(filePath);
		if (!pending) {
			return undefined;
		}
	...
	private async _persistEditSnapshot(
		turnId: string,
		toolCallId: string,
		filePath: string,
