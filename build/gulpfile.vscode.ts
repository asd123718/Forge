...
function getCodexExcludeFilter(platform: string, arch: string): string[] {
	const target = `${platform}-${arch}`;
	const packages = ['linux-x64', 'linux-arm64', 'darwin-x64', 'darwin-arm64', 'win32-x64', 'win32-arm64'];
	return [
		'**',
		...packages.filter(name => name !== target).map(name => `!**/node_modules/@openai/codex-${name}/**`),
	];
}
...
function packageTask(platform: string, arch: string, sourceFolderName: string, destinationFolderName: string, _opts?: { stats?: boolean }) {
	const destination = path.join(packageRoot, destinationFolderName);
	...
	const task = () => {
		...
		const codexTarget = `${platform}-${arch}`;
		const codexSdkFiles = gulp.src([
			'node_modules/@openai/codex/**',
			`node_modules/@openai/codex-${codexTarget}/**`,
		], { base: '.', dot: true, allowEmpty: true });
		const deps = es.merge(cleanedDeps, copilotRuntimePrebuilds, osProxyResolverPlatformPackage, codexSdkFiles)
			...
			.pipe(filter(getCodexExcludeFilter(platform, arch)))
			...
			.pipe(createAsar(path.join(process.cwd(), 'node_modules'), [
				...
				// Codex is spawned as a native subprocess from
				// `@openai/codex-<os>-<arch>/vendor/<triple>/bin/codex(.exe)`.
				// That binary cannot run from inside the ASAR archive.
				'**/@openai/codex-*/**',
...
function patchWin32DependenciesTask(destinationFolderName: string) {
	const cwd = path.join(packageRoot, destinationFolderName);
...
function prepareCopilotRipgrepShimTask(platform: string, arch: string, destinationFolderName: string) {
	const outputDir = path.join(packageRoot, destinationFolderName);
...
BUILD_TARGETS.forEach(buildTarget => {
	...
	const [vscode, vscodeMin] = ['', 'min'].map(minified => {
		...
		const packageTasks: task.Task[] = [
			...
			util.rimraf(path.join(packageRoot, destinationFolderName)),
...
task.task(task.define(
	...
	task.series(
		...
		function () {
			...
			).pipe(vfs.dest('.build/vscode-translations-export'));
		}
	)
));
...
task.task('vscode-translations-import', function () {
	const options = minimist(process.argv.slice(2), {
		string: 'location',
		default: {
			location: '.build/vscode-translations-import'
		}
	});
